...
public class MiniGameController {
	...
	// 퇴장하는 사람
	@PostMapping("/kickOut")
	public void kickOut(@RequestBody Map<String, Integer> payload) {
		int roomId = payload.get("roomId");
		int userId = payload.get("userId");
		Map<String, Object> map = new HashMap<>();
		map.put("roomId", roomId);
		map.put("userId", userId);
		miniGameService.kickOut(map);
	}
	
	@PostMapping("/checkKickOut")
	public Integer checkKickOut(@RequestBody Map<String, Integer> payload) {
		int roomId = payload.get("roomId");
		int userId = payload.get("userId");
		Map<String, Object> map = new HashMap<>();
		map.put("roomId", roomId);
		map.put("userId", userId);
		return miniGameService.checkKickOut(map);
	}
	...
	public Map<String, Object> leaveRoom(@RequestBody Map<String, Integer> payload) {
		...
		if (remainingGamers == null || remainingGamers.isEmpty()) {
			...
			miniGameService.deleteKickOut(roomId);
		}
		log.info("나가는 정보 : {}", map);
